# The ADC16 class definition in this file is incomplete.
#
# This file should not be require'd directly, but should instead be require'd
# through either `adc16/katcp` or `adc16/tapcp` depending on which protocol
# will be used to communicate with the board hosting the ADC16 devices.

# Provides wrapper around ADC16 based CASPER design.  Includes many
...
# (via the #adc16_controller method, which returns a `Bram` object).
#
...
class ADC16
  ...
  def initialize(*args)
    super(*args)
    @chip_select = 0xff
    @use_wb_ram = devices.grep(/adc16_wb_ram/).any?
  end
